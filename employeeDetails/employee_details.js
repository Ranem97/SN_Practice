...
function findEmployeeBySpecilization(specilization) {
  const foundEmployee = employees.find(
    (employee) => employee.specilization === specilization
  );
  if (foundEmployee) {
    document.getElementById(
      "employeesDetails"
    ).innerHTML = `<p>${foundEmployee.id}: ${foundEmployee.name}: - ${foundEmployee.department} - ${foundEmployee.specilization}</p>`;
  } else {
    document.getElementById("employeesDetails").innerHTML =
      "no employee has been found with this ID";
  }
